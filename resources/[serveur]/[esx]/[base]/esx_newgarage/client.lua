...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = "Rentrer le vehicule endommagé", value = 'yes'},
		{label = "Il est interdit de rentrer un véhicule abimé, faites le réparer.", value = 'no'},
	}
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if(data.current.value == 'no') then
				ESX.ShowNotification('Il est interdit de rentrer un véhicule abimé, faites le réparer.')
			end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				...
				TriggerEvent('esx:showNotification', 'Vous ne pouvez pas rentrer ce véhicule.')
			end
		end,vehicleProps)
	else
