...
--N'hesitez pas a rejoindre mon discord : 
...
local tacos = { x = 13.850054, y = -1599.36791, z = 28.37} --Configuration marker prise de service

local tacosfin = { x = 17.03042, y = -1612.93127, z = 28.28} --Configuration marker fin de service

local spawnfaggio = { x = 13.7474, y = -1591.95544, z = 28.08 } --Configuration du point de spawn du faggio
...
Citizen.CreateThread(function() --Thread lancement + livraison depuis le marker vert
  ...
  while true do
    ...
    if isInJobPizz == false then
      ...
      DrawMarker(1,tacos.x,tacos.y,tacos.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,0,255,0, 200, 0, 0, 0, 0)



      if GetDistanceBetweenCoords(tacos.x, tacos.y, tacos.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then

        HelpText("Appuyez sur ~INPUT_CONTEXT~ pour lancer la livraison de ~r~tacos",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
            ...
            distance = round(GetDistanceBetweenCoords(tacos.x, tacos.y, tacos.z, px,py,pz))
            ...
            TriggerServerEvent("tacos:itemadd", nbPizza)
    ...
    if isToHouse == true then
      ...
      while notif == true do
        ...
        TriggerEvent("pNotify:SendNotification", {
          ...
          text = "Direction : " ..destinol.. " pour livrer le tacos",
      ...
      if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 3 then
        ...
        HelpText("Appuyez sur ~INPUT_CONTEXT~ pour livrer le tacos",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsControlJustPressed(1,38) then
          ...
          afaitunetacosmin = true



          TriggerServerEvent("tacos:itemrm")
          ...
          if (posibilidad > 70) and (posibilidad < 90) then
            ...
            TriggerServerEvent("tacos:pourboire", pourboire)
    ...
    if isToPizzaria == true then
      ...
      while notif2 == true do
        ...
        SetNewWaypoint(tacos.x,tacos.y)



        TriggerEvent("pNotify:SendNotification", {

          text = "Direction le O'Tacos !",
      ...
      if GetDistanceBetweenCoords(tacos.x,tacos.y,tacos.z, GetEntityCoords(GetPlayerPed(-1),true)) < 3 and afaituntacosmin == true then

        HelpText("Appuyez sur ~INPUT_CONTEXT~ pour recuperer les tacos",0,1,0.5,0.8,0.6,255,255,255,255)
        ...
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("faggio2"))  then
          ...
          if IsControlJustPressed(1,38) then
            ...
            if IsInVehicle() then
              ...
              TriggerServerEvent("tacos:pourboire", paie)
...
Citizen.CreateThread(function() -- Thread de "fin de service" depuis le point rouge
  ...
  while true do
    ...
    if isInJobPizz == true then
      ...
      DrawMarker(1,tacosfin.x,tacosfin.y,tacosfin.z, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,0,0, 200, 0, 0, 0, 0)



      if GetDistanceBetweenCoords(tacosfin.x, tacosfin.y, tacosfin.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then

        HelpText("Appuyez sur ~INPUT_CONTEXT~ pour arreter la livraison du ~r~tacos",0,1,0.5,0.8,0.6,255,255,255,255)



        if IsControlJustPressed(1,38) then

          TriggerServerEvent('tacos:deleteAllPizz')
          ...
          if afaitunetacosmin == true then
            ...
            TriggerServerEvent("tacos:paiefinale")
