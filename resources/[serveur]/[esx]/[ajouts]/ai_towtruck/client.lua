...
function GoToTarget(x, y, z, truck, driver, truckhash, car)
	...
	ShowAdvancedNotification(companyIcon, companyName, "Appel fourrière passé.", "Un chauffeur se ramène, merci pour l'appel.")
	enroute = true
...
function PickupTarget(truck, driver, car)
	...
	ShowAdvancedNotification(towTruckDriverPick.icon, towTruckDriverPick.name, "Véhicule mis en fourrière" , towTruckDriverPick[1][math.random(#towTruckDriverPick[1])])
	Citizen.Wait(5000)
...
function GetTargetVehicle(player, dir)
	...
	if DoesEntityExist(towedVehicle) then
		...
		ShowNotification("Véhicule non détecté.")
	end
end
