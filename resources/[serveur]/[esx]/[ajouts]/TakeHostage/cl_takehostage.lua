...
function otage()
	...
	if not canotage then 
		drawNativeNotification("Tu n'es pas armé.")
	end
	...
	if not holdingHostageInProgress and canotage then		
		...
		if closestPlayer ~= -1 and closestPlayer ~= nil then
			...
			drawNativeNotification("Personne aux alentours.")
		end 
	end
	canotage = false 
end 
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Appuies sur [G] pour le libérer, [K] pour le tuer.")
